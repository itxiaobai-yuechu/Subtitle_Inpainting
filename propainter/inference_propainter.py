...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-i', '--video', type=str, help='Path of the input video or image folder.')
    parser.add_argument(
        '-m', '--mask', type=str, help='Path of the mask(s) or mask folder.')
    parser.add_argument(
    ...
    parser.add_argument(
        '--save_root', type=str, help='Save output videos.')
    ...
    # 加入原视频音频，覆盖原文件
    for fname in ['masked_in.mp4', 'inpaint_out.mp4']:
        video_path = os.path.join(save_root, fname)
        tmp_path = video_path.replace(".mp4", "_tmp.mp4")
        cmd = [
            "ffmpeg",
            "-y",                # 覆盖输出
            "-i", video_path,    # 新生成的视频（无声）
            "-i", args.video,    # 原视频（带音频）
            "-c:v", "copy",      # 直接拷贝视频流
            "-c:a", "aac",       # 音频转 aac
            "-map", "0:v:0",     # 取新视频的视频流
            "-map", "1:a:0",     # 取原视频的音频流
    ...
    os.replace(tmp_path, video_path)  # 用带音频的视频覆盖原文件
